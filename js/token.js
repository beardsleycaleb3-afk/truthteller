...
export function tokenize(raw) {
  ...
  return parts
    .map((part, i) => {
      const m = part.toUpperCase().match(/[A-Z]/);
      const letter = m ? m[0] : '';
      if (!letter) return null;
      const isLast = i === parts.length - 1;
      const finalEntry = isLast ? FINAL[letter] : undefined;
      const entry = finalEntry || REGULAR[letter];
      return {
        raw: part,
        letter,
        isLast,
        ...
        title: entry ? entry[0] : `Unknown Glyph (${letter})`,
